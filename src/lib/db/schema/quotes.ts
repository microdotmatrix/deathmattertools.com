...
export const SavedQuotesTable = pgTable(
  "saved_quotes",
  {
  ...
  (table) => [
    index("saved_quotes_entry_id_idx").on(table.entryId),
    index("saved_quotes_type_idx").on(table.type),
    index("saved_quotes_faith_idx").on(table.faith),
  ]
);
