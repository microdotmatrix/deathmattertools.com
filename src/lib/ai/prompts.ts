...
/**
 * Base formatting guidelines (uses medium length by default)
 * Used by assistantPrompt and analyzeDocumentPrompt for consistent defaults.
 */
export const promptFormating = `
  GENERAL STRUCTURE:\n
  - ${getLengthGuideline(DEFAULT_OBITUARY_LENGTH)}\n
...
/**
 * Generate formatting guidelines with dynamic length
 */
export const getPromptFormattingWithLength = (length: ObituaryLength): string => `
  GENERAL STRUCTURE:\n
...
export const assistantPrompt = `
  You are a virtual writing assistant that helps users review and revise obituaries generated by Death Matter Tools AI. Your task is to review the obituary and provide feedback on how to improve it, including any necessary revisions to the content, formatting, or structure. Follow the formatting guidelines, as outlined here:\n\n
...
/**
 * Few-Shot System Prompt (static version for backward compatibility)
 *
...
/**
 * Few-Shot System Prompt with dynamic length
 *
 * Creates a system prompt with the specified obituary length guideline.
 */
export const createFewShotSystemPrompt = (length: ObituaryLength): string => `
  You are a compassionate and eloquent obituary writer. Write respectful, heartfelt obituaries based strictly on provided facts.\n\n

  ${getPromptFormattingWithLength(length)}
