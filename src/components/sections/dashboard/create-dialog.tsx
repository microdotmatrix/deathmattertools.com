...
import { CreateEntryImage } from "./create-image";

export const CreatePortal = () => {
  const { open, setOpen } = useCreateForm();
  const { images } = useEntryImage();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent
        className="max-w-[90%] md:max-w-7xl max-h-[90vh] overflow-y-auto mx-auto p-0 gap-0"
        aria-describedby="create-entry-description"
      >
        <div
          className="flex flex-col md:flex-row gap-8 items-start md:items-center overflow-y-auto flex-1 min-h-0 lg:overflow-y-clip p-4 lg:p-8"
          style={
            isMobile
              ? {}
              : {
        ...
        >
          ...
          <div className="w-full md:w-1/2 lg:w-2/3">
            <CreateEntryImage uploadedImages={images} />
          </div>
        </div>
      </DialogContent>
    </Dialog>
